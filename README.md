<div align="center">

# Aryan Satyendra Kumar

### ECE Undergraduate at NIT Goa · Minor in Computer Science

**Machine Learning · Computer Vision · Multimodal AI · RAG Systems**

<p align="center">
  <a href="mailto:kumararyan66472@gmail.com">
    <img src="https://img.shields.io/badge/Email-kumararyan66472%40gmail.com-0e6666?style=flat-square&logo=gmail&logoColor=white" alt="Email">
  </a>
  <a href="https://www.linkedin.com/in/kumaryan12">
    <img src="https://img.shields.io/badge/LinkedIn-kumaryan12-0e6666?style=flat-square&logo=linkedin&logoColor=white" alt="LinkedIn">
  </a>
  <a href="https://portfolio-ochre-zeta-28.vercel.app">
    <img src="https://img.shields.io/badge/Portfolio-Visit-3acebc?style=flat-square&logo=vercel&logoColor=06181c" alt="Portfolio">
  </a>
  <img src="https://komarev.com/ghpvc/?username=Kumaryan12&style=flat-square&color=0e6666&label=PROFILE+VIEWS" alt="Profile views">
</p>

## About me

I am an **Electronics and Communication Engineering undergraduate at NIT Goa**, pursuing a **Minor in Computer Science**. I build applied machine-learning systems at the intersection of **multimodal perception, computer vision, NLP/RAG, environmental intelligence, and dependable software engineering**.

My work ranges from leakage-aware PM2.5 modelling and visual road-dust analysis to source-grounded legal assistance, probabilistic Formula 1 simulation, and civic-tech platforms designed around real operational workflows.


<table>
<tr>
<td width="50%" valign="top">

### Current direction

- Multimodal and temporal machine learning
- Computer vision for real-world sensing
- Reliable evaluation and leakage auditing
- Retrieval-augmented generation with guardrails
- End-to-end ML products using FastAPI and Next.js

</td>
<td width="50%" valign="top">

### Currently building

- Roadside air-quality intelligence
- Source-grounded public-service AI
- Probabilistic sports simulations
- Civic-tech decision-support systems
- Human-in-the-loop operational dashboards

</td>
</tr>
</table>

## Research and experience

**Project Intern, IIT Madras — Mobile Air Quality Intelligence**  
Developed a multimodal roadside PM2.5 pipeline combining mobile sensors, OPC particle-size bins, atmospheric products, meteorology, GPS/OSM context, and camera imagery. Worked with YOLO, SegFormer, ResNet50-GRU, tree-based models, whole-date validation, and a filter-sampling/XRF validation workflow.

**Signalling and Telecommunications Intern — South Western Railways / RVNL**  
Studied Kyosan Electronic Interlocking in a safety-critical environment, including command-to-field feedback paths, relay interfaces, interlocking logic, and failure-mode handling.



## Technical toolkit

<p align="center">
  <img src="https://skillicons.dev/icons?i=python,cpp,c,ts,js,pytorch,sklearn,fastapi,nextjs,react,postgres,firebase,git,linux,vscode&perline=15" alt="Technical skills">
</p>

<table>
<tr>
<td><strong>Machine learning</strong></td>
<td>PyTorch, scikit-learn, Random Forests, feature engineering, cross-validation, temporal backtesting, uncertainty modelling</td>
</tr>
<tr>
<td><strong>Vision and multimodal AI</strong></td>
<td>YOLO, SegFormer, CNNs, ResNet, VGG, LSTMs, GRUs, image/text embeddings</td>
</tr>
<tr>
<td><strong>NLP and LLM systems</strong></td>
<td>Transformers, XLM-R, Sentence Transformers, RAG, ChromaDB, retrieval evaluation, grounded generation</td>
</tr>
<tr>
<td><strong>Backend and data</strong></td>
<td>FastAPI, REST APIs, PostgreSQL, Firestore, Firebase, Supabase, Pandas, NumPy</td>
</tr>
</table>

## Selected milestones

- Selected for **Amazon ML Summer School 2026**
- Ranked **251 out of 7,000+ teams** in the Amazon ML Challenge 2025
- Finalist at **India Innovates 2026**
- Finalist in multiple national case-study competitions across IIT, IIM, and NIT platforms
- GDG Organizer and AI/ML Lead; led a 100-participant hackathon and conducted ML and DSA workshops
- Captain of the NIT Goa badminton team and former national-level badminton player

## GitHub activity

<p align="center">
  <img
    width="48%"
    src="./profile-summary-card-output/github_dark/3-stats.svg"
    alt="GitHub statistics"
  />

  <img
    width="48%"
    src="./profile-summary-card-output/github_dark/1-repos-per-language.svg"
    alt="Languages by repository"
  />
</p>

<p align="center">
  <img
    width="97%"
    src="./profile-summary-card-output/github_dark/0-profile-details.svg"
    alt="GitHub contribution activity"
  />
</p>
---




